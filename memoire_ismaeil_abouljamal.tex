...
\includegraphics[scale=.5]{images/treillis3}&\includegraphics[scale=.5]{images/treillis4}\\
...
\includegraphics[scale=.5]{images/treillis5}&\includegraphics[scale=.5]{images/treillis6}\\
 \'Elimination de la feuille 1 & \'Elimination de la feuille 3\\
\includegraphics[scale=.5]{images/treillis7}&\includegraphics[scale=.5]{images/treillis8}\\
...
On décrit le fonctionnement de cet algorithme avant de présenter son impléme\-ntation en python dans le chapitre \ref{implement} section \ref{ordo}


\begin{itemize}

 \item Dans chaque étape de l'algorithme, on choisi une ligne-pivot parmi les lignes autorisées si elle est maximale (c'est-à-dire si l'ensemble de ses attributs 
est maximal au sens de l'inclusion). Cette ligne-pivot est alors déplacée en bas du tableau et les colonnes de la table qui font partie des colonnes autorisées 
sont réordonnées de façon à avoir les 1 de cet individu à droite et ses 0 à gauche.
...
de la dernière ligne-pivot choisie.
 ...
 \item Une fois le parcourt montant des lignes a été effectué, l'étape de descente de l'algorithme commence pour traiter les blocs non scindés par l'étape montante 
. Ce dernier traitement se fait de droite à gauche et de bas en haut jusqu'à ce que tous les blocs soient traités.
\end{itemize}

Cet algorithme est implémenté de manière interactive en python dans le chapitre \ref{implement} section \ref{ordo}
...
\section{Implémentation de l'algorithme de réordonnancement}

\label{ordo}

Le code source de cet algorithme est également disponible sur Github \lien{https://github.com/ismaeil/ALGO}

Pour accompagner l'exécution de l'algorithme pas à pas et obtenir les résultats intermédiaire du réordonnacement, une version de l'algorithme 
utilisant la bibliothèque curses \lien{http://docs.python.org/release/2.6/howto/curses.html} est disponible sur Github \lien{https://github.com/ismaeil/ALGO_CURSES}

\textbf{Fichier $table.py$}
\begin{lstlisting}
#! /usr/bin/python
# -*- coding: utf-8 -*-
...
La question de reconnaissance d'ordonnabilité sur les tables démonta\-bles est résolue par l'algorithme d'ordonnancement proposé dans la section \ref{algor}. 
On peut directement ordonner une table ordonnable grâce à cet algorithme. De plus l'ordre finale sur les lignes de la table donne un ordre de démontabilité 
sur les le treillis associé à la table.
